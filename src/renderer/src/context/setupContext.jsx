...
export const SetupProvider = ({ children }) => {
  ...
  useEffect(() => {
    const wifiAndPasswordIsFilled =
      selectedWifi && wifiPassword && selectedWifi.length > 0 && wifiPassword.length > 0
  ...
  const connectDeviceToSelectedWifi = async () => {
    ...
    try {
      ...
      const res = await runCommandForDevice(
        '10.1.2.3',
        `/wifiapi?ssid=${selectedWifi}&type=WPA2&key=${wifiPassword}`,
        'GET'
      )
  ...
  const runPlayersSetupProcess = async (discoveredDevices) => {
    ...
    matrix = await Promise.all(
      matrix.map(async (device) => {
        ...
        if (thisDevice && thisDevice.ip === '10.1.2.3' && !device.wifiAvailable) {
          ...
          !device.wifiAvailable &&
          wifiRef &&
          wifiRef.current &&
          wifiRef.current === device.name
        ) {
          // device.mac = await getMacAddress("10.1.2.3")
          const res = await checkWifiAvailableOnPlayer()
          device.wifiAvailable = res
    ...
    const additionalMatrixItems = additionalDevices
      .map((device) => {
  ...
  const createMatrixAndStart = () => {
    setSetupMatrix(needSetupDevices.map((device) => createMatrixItem(device)))
    setInProgress(true)
  }
