...
const AddPlayerButton = () => {
  ...
  const getWifi = async () => {
    ...
    const allWifiList = wifiList
      .map((wifi) => {
        if (wifi.ssid.length === 0) {
          return null
        }
        return isWifiFormatCorrect(wifi.ssid, wifi.security)
          ? { ...wifi, correctFormat: true }
          : { ...wifi, correctFormat: false }
      })
      .filter((wifi) => wifi !== null)

    setBluosDevicesList(
      allWifiList.map((wifi) => (wifi.correctFormat ? wifi : null)).filter((wifi) => wifi !== null)
    )
    setWifiList(
      allWifiList.map((wifi) => (wifi.correctFormat ? null : wifi)).filter((wifi) => wifi !== null)
    )
  }
    ...
    >
      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="w-full rounded-xl flex flex-col justify-center gap-1">
          {inProgress && (
            <div className="border border-accent rounded-md bg-accent px-2 py-1">
              {bluosDevicesList &&
              bluosDevicesList.filter((wifi) => !isDeviceSelected(wifi.ssid)).length > 0 ? (
                bluosDevicesList
                  .filter((wifi) => {
                  ...
                  .map((wifi) => {
                    return (
                      <div className="w-full p-3 flex items-center gap-3" key={wifi.ssid}>
                        <PlusCircle
                          className="w-4 h-4 cursor-pointer  text-primary/50 hover:text-primary"
                          onClick={() => {
                            addToAdditionalDevices(wifi.ssid)
                          }}
                        />
                ...
                <div className="flex items-center gap-2">
                  ...
                  Searching for more BluOS Devices...
                </div>
              )}
            </div>
          )}

          {inProgress && setupMatrix && setupMatrix.length > 0
            ? setupMatrix.map((device) => {
                return (
                  <div className="flex gap-2 text-xs p-2 border border-accent rounded-md  bg-accent ">
                    ...
                    <XCircle
                      className="w-4 h-4 cursor-pointer  text-primary/50 hover:text-red-500 duration-300 ease-in transition-colors"
                      onClick={() => removeItemFromMatrix(device.name)}
                    />
                  </div>
                )
              })
          ...
          <>
            {bluosDevicesList && bluosDevicesList.length > 0 ? (
              bluosDevicesList.map((wifi) => {
                return (
                  <div
                    className="w-full rounded-md p-3 cursor-pointer flex items-center gap-3 bg-accent "
                    key={wifi.ssid}
                    onClick={() =>
                      isDeviceSelected(wifi.ssid)
                        ? deselectDevice(wifi.ssid)
                        : selectDevice(wifi.ssid)
                    }
                  >
                    <Checkbox
                      checked={isDeviceSelected(wifi.ssid)}
                      onCheckedChange={(e) =>
                        e ? selectDevice(wifi.ssid) : deselectDevice(wifi.ssid)
                      }
                    />
                    <p className="text-xl">{wifi.ssid}</p>
                  </div>
              ...
              <div className="flex flex-col items-center justify-center py-14">
                ...
                <p className="text-xl">Searching For BluOS Device... {timer}</p>
        ...
        {!inProgress && (
          <>
            {wifiList && wifiList.length > 0 ? (
              ...
              <p>Looking for available Wifi Network...</p>
            )}
          </>
        )}
      </DialogContent>
