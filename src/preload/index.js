...
const api = {
  ...
  playerControl: (ip, control, param) =>
    ipcRenderer.invoke('player-control', { ip, control, param }),
  ...
  runCommandForDevice: (ip, command, type, body) =>
    ipcRenderer.invoke('run-command-for-device', { ip, command, type, body }),
  loadSDUIPage: (url, debug, schema) =>
    ipcRenderer.invoke('load-sd-ui-page', { url, debug, schema }),
  connectToDeviceThroughWifi: (ssid, password) =>
    ipcRenderer.invoke('connect-to-wifi', { ssid, password }),
